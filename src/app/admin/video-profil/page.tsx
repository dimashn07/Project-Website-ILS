...
import { deleteVideoProfil, getVideoProfil } from '@/controller/videoprofil';

const VideoProfilPage = () => {
    ...
    const handleTambahClick = () => {
      router.push('admin/video-profil/tambah');
    };

    const handleUbahClick = (profil) => {
      router.push(`admin/video-profil/ubah?id=${profil.id}`);
    };

    useEffect(() => {
      async function getData() {
        const profil = await getVideoProfil(session);
        setVideo(profil);
      }
      getData();
    }, [session]);
                  ...
                  <td className="px-6 py-4 justify-center">
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        ...
                        if (isConfirmed) {
                          const deletedVideo = await deleteVideoProfil(item.id, item.gambar, session);
                          if (deletedVideo) {
                            const updatedVideo = video.filter((t) => t.id !== deletedVideo);
...
export default VideoProfilPage;